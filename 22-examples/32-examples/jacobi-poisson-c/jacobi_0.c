  ...
  for (k = 0; k < step; k++)
    {
      if (myrank == 0)
	{
	  //根进程首先向上方发送本地"真矩阵"的第1行, 
	  //然后从下方接收并填充本地"虚矩阵"的最后一行.
	  MPI_Send (mat_local[1], col_local, MPI_DOUBLE, upper, 0,
		    MPI_COMM_WORLD);
	  MPI_Recv (mat_local[row_local - 1], col_local, MPI_DOUBLE, down, 0,
		    MPI_COMM_WORLD, &status);

	  //根进程首先向下方发送本地"真矩阵"的最后一行, 
	  //然后从上方接收并填充本地"虚矩阵"的第1行.
	  MPI_Send (mat_local[row_local - 2], col_local, MPI_DOUBLE, down, 1,
		    MPI_COMM_WORLD);
	  MPI_Recv (mat_local[0], col_local, MPI_DOUBLE, upper, 1,
		    MPI_COMM_WORLD, &status);
	}
      else
	{
	  //非根进程首先从下方接收并填充本地"虚矩阵"的最后一行,
	  //然后向上方发送本地"真矩阵"的第1行.
	  MPI_Recv (mat_local[row_local - 1], col_local, MPI_DOUBLE, down, 0,
		    MPI_COMM_WORLD, &status);
	  MPI_Send (mat_local[1], col_local, MPI_DOUBLE, upper, 0,
	  ...
	  //非根进程首先从上方接收并填充本地"虚矩阵"的第1行,
	  //然后将本地"真矩阵"的最后一行向下方发送.
	  MPI_Recv (mat_local[0], col_local, MPI_DOUBLE, upper, 1,
		    MPI_COMM_WORLD, &status);
	  MPI_Send (mat_local[row_local - 2], col_local, MPI_DOUBLE, down, 1,
		    MPI_COMM_WORLD);
